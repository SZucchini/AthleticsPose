"""Graph module from TaatiTeam/MotionAGFormer repository (Apache-2.0 License)."""
...
class GCN(nn.Module):
    ...
    def _init_temporal_adj(self, connection_length):
        """Connect each joint to itself and the same joint withing next `connection_length` frames."""
        adj = torch.zeros((self.num_nodes, self.num_nodes))
